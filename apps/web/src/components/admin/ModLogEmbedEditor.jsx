...
export default function ModLogEmbedEditor({ templateId, embed, onChange, onResetDefault }) {
  const baseEmbed = useMemo(
    () => mergeModLogEmbedForEditor(embed, templateId),
    [embed, templateId],
  )
  ...
  const placeholders = PLACEHOLDERS_BY_TEMPLATE[templateId] ?? MOD_LOG_MESSAGE_PLACEHOLDERS
  const previewCtx = PREVIEW_BY_TEMPLATE[templateId] ?? MOD_LOG_PREVIEW_MESSAGE_DELETE
  ...
  function emit(next) {
    onChange(mergeModLogEmbedForEditor(next, templateId))
  }
  ...
  return (
    <div className="grid items-start gap-6 lg:grid-cols-[minmax(0,1fr)_auto]">
      <div className="min-w-0 space-y-4">
        ...
        <CollapsibleSection
          title="Embed message"
          subtitle="Title, description, color, and field rows (most log content lives in fields)."
        >
          <div className="mb-3 flex justify-end">
            <button
              type="button"
              onClick={() => {
                const defaults = DEFAULT_MOD_LOG_EMBEDS[templateId] ?? EMPTY_EMBED
                emit({
                  ...defaults,
              ...
              <div className={`${adminInset} space-y-4 p-4`}>
                ...
                <div className="grid gap-3 sm:grid-cols-2">
                  <Field
                    ...
                    placeholder="{avatar_url} for members"
                  />
                  <Field
                    label="Image URL"
                ...
                <div className="grid gap-3 sm:grid-cols-2">
                  <Field
                    ...
                    placeholder="Requested by {requested_by}"
                  />
                  <Field
