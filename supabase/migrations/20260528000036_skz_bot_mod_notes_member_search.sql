-- Guild member directory for mod notes search (populated on Discord cache sync).

ALTER TABLE skz_admin_discord_member_roles_cache
  ADD COLUMN IF NOT EXISTS username text NOT NULL DEFAULT '',
  ADD COLUMN IF NOT EXISTS avatar_url text NOT NULL DEFAULT '';

CREATE INDEX IF NOT EXISTS skz_admin_discord_member_roles_cache_search_idx
  ON skz_admin_discord_member_roles_cache (guild_id, lower(display_name), lower(username));
...
BEGIN
  ...
  WHERE m.guild_id = COALESCE(NULLIF(trim(v_guild_id), ''), m.guild_id)
    AND (
      ...
      OR lower(m.username) LIKE '%' || v_query || '%'
...
GRANT EXECUTE ON FUNCTION skz_admin_bot_search_guild_members(text, text, int, int) TO anon, authenticated;
