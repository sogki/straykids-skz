-- Allow explicit QOTD test runs through the admin outbox queue.
...
ALTER TABLE skz_bot_outbox
  ...
  CHECK (action IN ('DEPLOY_MESSAGE', 'SYNC_GUILD_CACHE', 'RUN_DAILY_QUESTION_NOW'));
...
BEGIN
  ...
  IF p_action NOT IN ('DEPLOY_MESSAGE', 'SYNC_GUILD_CACHE', 'RUN_DAILY_QUESTION_NOW') THEN
