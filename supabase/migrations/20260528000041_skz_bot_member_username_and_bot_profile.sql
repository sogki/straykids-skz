-- Member search: never treat display_name as login username.
-- Bot profile keys: populated by Discord cache sync for admin embed previews.

INSERT INTO skz_bot_settings (key, value, description) VALUES
  ('bot_discord_user_id', '', 'Discord user id of this bot application (synced)'),
  ('bot_username', '', 'Bot login username without @ (synced)'),
  ('bot_global_name', '', 'Bot display / global name (synced)'),
  ('bot_avatar_hash', '', 'Bot avatar hash for CDN URLs (synced)')
