-- Backfill login usernames from discord_cache.extra, then expose them reliably in member search.

UPDATE skz_admin_discord_member_roles_cache m
SET username = src.login_name
FROM (
  SELECT
    c.entity_id AS discord_user_id,
    c.guild_id,
    NULLIF(trim(c.extra->>'username'), '') AS login_name
  FROM skz_bot_discord_cache c
  WHERE c.entity_type = 'member'
    AND NULLIF(trim(c.extra->>'username'), '') IS NOT NULL
) src
WHERE m.discord_user_id = src.discord_user_id
  AND m.guild_id = src.guild_id
  AND COALESCE(NULLIF(trim(m.username), ''), '') = '';
...
BEGIN
  ...
  WHERE m.guild_id = COALESCE(NULLIF(trim(v_guild_id), ''), m.guild_id)
    AND (
      ...
      OR lower(COALESCE(
        (SELECT c.name FROM skz_bot_discord_cache c
         WHERE c.guild_id = m.guild_id AND c.entity_type = 'member' AND c.entity_id = m.discord_user_id
         LIMIT 1),
        ''
      )) LIKE '%' || v_query || '%'
      OR lower(COALESCE(
        (SELECT c.extra->>'username' FROM skz_bot_discord_cache c
         WHERE c.guild_id = m.guild_id AND c.entity_type = 'member' AND c.entity_id = m.discord_user_id
         LIMIT 1),
        ''
      )) LIKE '%' || v_query || '%'
      ...
      FROM (
        SELECT
          ...
          COALESCE(
            ...
            NULLIF(trim(ln.target_username), ''),
            ''
          ) AS username,
          COALESCE(
        ...
        LEFT JOIN skz_player_profiles pp
          ON pp.discord_user_id = m.discord_user_id
        LEFT JOIN LATERAL (
          SELECT n.target_username
          FROM skz_bot_mod_notes n
          WHERE n.guild_id = m.guild_id
            AND n.target_discord_user_id = m.discord_user_id
            AND n.deleted_at IS NULL
            AND NULLIF(trim(n.target_username), '') IS NOT NULL
        ...
        ORDER BY
          lower(COALESCE(c.extra->>'display_name', m.display_name)),
          lower(COALESCE(c.extra->>'username', m.username, pp.username, ''))
        LIMIT v_limit
        OFFSET v_offset
      ) r
    ), '[]'::jsonb),
